...
import { Rocket, ArrowRight, ArrowLeft, Sparkles, Shield, Zap, Check } from 'lucide-react';
...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (password.length < 6) {
            setError(t('يجب أن تكون كلمة المرور 6 أحرف على الأقل', 'Password must be at least 6 characters long'));
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                setError(result.error || t('فشل إنشاء الحساب', 'Registration failed'));
            }
        } catch {
                    ...
                    <Card.Header className="space-y-2 text-center pt-8 pb-4">
                        <div className="flex justify-center mb-2">
                            <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-primary/20 bg-primary/5 text-sm">
                                ...
                                <span className="text-muted-foreground">{t('انضم إلينا', 'Join us')}</span>
                            </span>
                        </div>
                        <Card.Title className="heading-display text-3xl">{t('إنشاء حساب', 'Create Account')}</Card.Title>
                        <Card.Description className="text-muted-foreground">
                            {t('أدخل معلوماتك للبدء', 'Enter your information to get started')}
                        </Card.Description>
                    </Card.Header>

                    <form onSubmit={handleSubmit}>
                        <Card.Content className="space-y-4 px-8">
                            {error && (
                                <Alert status="danger">
                                    <Alert.Indicator />
                                    <Alert.Content>
                            ...
                            >
                                ...
                                <Input
                                    placeholder={t('أكّد كلمة المرور', 'Confirm your password')}
                                    className="h-12 rounded-xl"
                                />
                            </TextField>
                        </Card.Content>
                            ...
                            <div className="relative group w-full">
                                ...
                                >
                                    ...
                                    <span className="relative z-10">
                                        {isLoading ? t('جارٍ إنشاء الحساب...', 'Creating account...') : t('إنشاء حساب', 'Create account')}
                                    </span>
                                </Button>
                            </div>
